...
你不需要做全量的测试和检查，当你认为你的修改已经完成时，就请提交并 push 当前分支。push 成功后立即输出最终结果并结束本次 agent 任务。不得等待、轮询、重跑或尝试触发 push 后的新 CI；新 CI 的等待和后续派发由 dshw 负责。你不需要再做其他修改，也无需检查 base branch 是否又发生了新的提交，无需检查 review 意见。
